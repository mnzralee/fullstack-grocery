...
- [The Exception Handling Architecture](#the-exception-handling-architecture---errors-as-communication)
...
We're using multi-file schemas. Prisma reads every `.prisma` file in the `schema/` directory automatically. This became stable in Prisma 5.15 and is the default in Prisma 6.x, so if you're on `prisma@^5.15.0` or later, it just works. If you're on an earlier 5.x version, add `previewFeatures = ["prismaSchemaFolder"]` to the generator block. Check your version and move on.
...
export class InvalidItemStateError extends DomainError {
  constructor(currentStatus: string, targetStatus: string, operation: string) {
    super(
      'INVALID_ITEM_STATE',
      400,
      `Cannot ${operation} item: current status is ${currentStatus}, cannot transition to ${targetStatus}`,
...
This is the state machine enforcement error. When someone tries to approve a BOUGHT item, this error fires with the exact context: what the status is now, what status was attempted, and what operation triggered it.
...
describe('Domain Errors', () => {
  ...
  it('should create InvalidItemStateError with transition context', () => {
    const error = new InvalidItemStateError('BOUGHT', 'APPROVED', 'approve');
...
`(req as any).user?.id`: we access the user from the request. The auth middleware (Chapter 12) will attach this and introduce an `AuthenticatedRequest` type that replaces the `(req as any)` cast with proper type safety. For now, we fallback to 'anonymous'. Ugly? Yes. But *Ship it, then perfect it.* We'll clean this up when we add auth.
...
## The Exception Handling Architecture - Errors as Communication

> *"An error is not a failure. It's a message. A well-thrown error tells the next developer exactly what went wrong, where it went wrong, and what they should have done instead. A swallowed error tells them nothing — until production breaks at 2 AM and nobody knows why."*

We've built four layers. Domain, Application, Infrastructure, Interface. In each one, we wrote error handling code — the `DomainError` base class (Chapter 4), the `assertTransition` guard on state changes (Chapter 4), the validation middleware (Chapter 7), the error handler middleware (Chapter 7), the controller's `try/catch/next` pattern (Chapter 7). Each piece was introduced where it naturally belonged.

Now let me show you how all those pieces connect into a single, unified error architecture. This isn't a new chapter. It's a pause. A moment to see the complete picture before we wire everything together in Chapter 8.

### The Life of an Error

Let's trace a real error through every layer. Sarah (MANAGER) tries to approve a grocery item that Alex already bought. The item's status is BOUGHT. Watch what happens:

```
Sarah clicks "Approve" on Milk (status: BOUGHT)
         │
    ┌────▼─────────────────────────────────────────────────────────┐
    │  ① Domain Layer                                              │
    │     assertTransition('BOUGHT', 'APPROVED', 'approve')        │
    │     → throws InvalidItemStateError                           │
    │       code: 'INVALID_ITEM_STATE'                             │
    │       statusCode: 400                                        │
    │       message: 'Cannot approve item: current status is        │
    │                 BOUGHT, cannot transition to APPROVED'        │
    │       details: { currentStatus: 'BOUGHT',                    │
    │                  targetStatus: 'APPROVED',                    │
    │                  operation: 'approve' }                       │
    └────┬─────────────────────────────────────────────────────────┘
         │  error bubbles up — use case has no try/catch
    ┌────▼─────────────────────────────────────────────────────────┐
    │  ② Application Layer                                         │
    │     ApproveItemUseCase.execute()                              │
    │     → does NOT catch it, error passes straight through        │
    └────┬─────────────────────────────────────────────────────────┘
         │  error bubbles up — controller catches it
    ┌────▼─────────────────────────────────────────────────────────┐
    │  ③ Interface Layer — Controller                               │
    │     catch (error) { next(error) }                             │
    │     → delegates to Express error handling chain               │
    └────┬─────────────────────────────────────────────────────────┘
         │  Express calls the error handler middleware
    ┌────▼─────────────────────────────────────────────────────────┐
    │  ④ Interface Layer — Error Handler Middleware                  │
    ...
    │     → reads response.status (400), extracts error message     │
    │     → forwards to the browser                                 │
    └────┬─────────────────────────────────────────────────────────┘
         │
    ┌────▼─────────────────────────────────────────────────────────┐
    │  ⑥ Frontend — apiClient                                      │
    │     → !response.ok → throw new ApiError(message, 400)         │
    └────┬─────────────────────────────────────────────────────────┘
         │
    ┌────▼─────────────────────────────────────────────────────────┐
    │  ⑦ Frontend — React Component                                 │
    │     → TanStack Query catches → isError = true                 │
    │     → User sees: "Cannot approve item: status is BOUGHT,      │
    │                    must be PENDING"                            │
    └──────────────────────────────────────────────────────────────┘
```

Seven steps. Each one does exactly one thing. The domain throws. The use case lets it pass. The controller delegates. The middleware translates. The BFF forwards. The API client wraps. The component displays. Nobody swallows. Nobody guesses. Nobody adds a fallback that hides the bug.

The error message that Sarah sees in the browser is the *exact same message* that the domain layer generated. It traveled through seven layers without being altered, translated, or lost. That's the payoff.

### Three Categories of Errors

Not all errors are the same. Our system handles three distinct categories, and each one follows a different path:

**Domain Errors** — business rule violations. These are the errors we defined in Chapter 4: `ItemNotFoundError`, `ListNotFoundError`, `InvalidItemStateError`, `BudgetExceededError`, `InsufficientPermissionsError`. Each one extends `DomainError`, has a specific code, a meaningful message, and structured details. The error handler middleware recognizes them by `instanceof DomainError` and returns the appropriate HTTP status with the full error payload.

**Validation Errors** — input issues caught before business logic runs. These come from Zod in the validation middleware we built in Chapter 7. When a request body fails schema validation, the middleware returns a 400 with field-level errors. The controller never sees them. The use case never runs. The error is caught at the gate.

**Infrastructure Errors** — unexpected failures. A database connection drops. A query times out. A disk fills up. We don't define custom classes for these because they aren't business rules — they're operational failures. They hit the `else` branch in our error handler: the full error gets logged for the developer, and the user sees a generic "An unexpected error occurred" with a 500 status. No stack traces, no internal paths, no database error messages.

| Category | Defined By | Caught By | HTTP Status | User Sees |
|----------|-----------|-----------|-------------|-----------|
| Domain Error | Your error classes | Error handler middleware | 400 / 403 / 404 | Specific message + details |
| Validation Error | Zod | Validation middleware | 400 | Field-level errors |
| Infrastructure Error | Runtime | Error handler (else branch) | 500 | Generic message |

This table is the complete error taxonomy of our system. Every error that can occur falls into one of these three categories. If you're adding a new error and it doesn't fit, you're probably creating a fourth category that shouldn't exist.
...
It looks defensive. It looks safe. It's neither.

If the item doesn't exist, something is wrong. Either the ID is invalid, or your data is corrupted, or there's a race condition between deleting and reading. The `?? 'Unknown Item'` mask hides all three problems behind a silent default. You won't discover the bug until a user reports "Why does my grocery list say Unknown Item?" — and by then, you have no stack trace, no timestamp, no context. You have a mystery.
...
// Now 'item' is guaranteed to exist. TypeScript knows it too.
```

Explicit failure. The error includes the item ID, a timestamp, and a clean stack trace. When it fires in development, you see it immediately. You fix it immediately. It never reaches production.

Every code path should either succeed explicitly or throw explicitly. If you're writing `?? defaultValue` in business logic, stop and ask: *under what circumstances would this value actually be missing?* If the answer is "it shouldn't be," then throw an error. Defaults belong in configuration and UI. Not in business logic.

If you can't handle it, don't catch it. Let it bubble. Someone upstream will handle it — that's why we built the error handler middleware.

### The Error Contract: What Each Layer Is Allowed To Do

Each layer has a strict contract for how it handles errors. Violate these contracts and your error architecture breaks down.

**Domain Layer — throws, never catches.** The domain defines errors and throws them. `assertTransition` throws `InvalidItemStateError`. The `Money` value object would throw if you tried to create a negative amount. The domain never catches errors from other layers because it doesn't depend on other layers. If a business rule is violated, throw. Period.

**Application Layer (Use Cases) — orchestrates the happy path, never catches.** Look at every use case we wrote in Chapter 5. Not a single `try/catch`. The use case calls the domain and the repository. If either throws, the error passes straight through. The use case's job is to orchestrate success. Errors are someone else's problem.

You might be tempted to catch a `ListNotFoundError` in a use case and return `null`. Don't. Returning `null` from a use case is a silent failure. The controller won't know if `null` means "no data found" or "something went terribly wrong." An exception is unambiguous.

**Infrastructure Layer — implements, lets most errors bubble.** The repository doesn't catch Prisma errors and wrap them in domain errors. If `prisma.groceryItem.findUnique` throws a connection error, it bubbles through the use case and controller to the error handler, which returns a 500. That's the correct behavior: an infrastructure failure is not a business error. It's an operational failure, and the error handler already knows how to handle it.

**Interface Layer (Controller) — delegates with `next(error)`, never handles.** Every controller method follows the same pattern: `try { ... } catch (error) { next(error) }`. The controller never inspects the error. It never checks `if (error instanceof ItemNotFoundError)`. It never returns a custom message. It passes every error to Express's error handling chain and moves on.

**Interface Layer (Error Handler Middleware) — the single point of translation.** This is the ONLY place in your entire backend that converts errors to HTTP responses. One function. One file. If you need to change how errors are formatted — add a request ID, switch to a different JSON structure, add localization — you change one file. Not twenty controllers.

**BFF Layer — forwards status and message, adds no error logic.** The BFF reads the HTTP status from the backend response and forwards it to the browser. It doesn't add error handling. It doesn't retry. It doesn't transform error messages. It's a passthrough.

**Frontend — displays specific messages, never swallows.** The `ApiError` class carries the HTTP status code so components can handle 404 differently from 500. TanStack Query catches the error and sets `isError = true`. The component renders the actual error message — the one that originated in the domain layer, seven steps ago.

### Testing Error Paths

Most developers test the happy path and call it done. Your error paths need tests too — they're the paths your users will actually hit.

We already tested domain errors in Chapter 4: verifying that `assertTransition` throws `InvalidItemStateError` when the state transition is invalid, and that `Money.subtract` throws when the result would be negative. But there's one critical piece we haven't tested yet: the error handler middleware itself.

```typescript
describe('errorHandler', () => {
  it('should return structured response for DomainError', () => {
    const err = new ItemNotFoundError('item-123');
    ...
    expect(res.json).toHaveBeenCalledWith({
      ...
      error: expect.objectContaining({
        code: 'ITEM_NOT_FOUND',
        message: 'Grocery item not found: item-123',
      }),
    });
  });

  it('should return generic 500 for unexpected errors', () => {
...
Two tests. One for domain errors, one for unexpected errors. The error handler is the funnel everything passes through. If it's wrong, every error response is wrong. Test the funnel.

### Common Anti-Patterns

Before we move on, let me catalog the mistakes I see most often. If you recognize any of these in your own code, fix them.

**Catching and swallowing.** The caller has no idea the operation failed. Data is silently lost.

```typescript
// Anti-pattern: the error disappears
try { await riskyOperation(); } catch (e) { console.log(e); }
```

**Returning null instead of throwing.** Was the item not found, or did the database crash? The caller can't tell. Both return `null`.

```typescript
// Anti-pattern: silent null
async findItem(id: string): Promise<Item | null> {
  try {
    return await this.repo.findById(id);
  } catch {
    return null;  // Hides infrastructure failure behind "not found"
  }
}
```

**Generic error classes.** If every error is `AppError('Something went wrong', 400)`, the frontend can't distinguish between a budget overrun and an invalid state transition. The user sees the same message for both. The developer gets the same error code for both. Nobody can debug anything.

**try/catch in use cases.** If you find yourself writing `try/catch` in a use case, ask why. The use case should throw errors, not handle them. The only exception is if you need to enrich an error with additional context before re-throwing — and even then, think twice.

**"Something went wrong" for everything.** If your frontend shows the same message for a 400 and a 500, you wasted all the effort we put into structured errors. Use the error code. Switch on it. Show "Item not found" for 404, "Budget exceeded" for `BUDGET_EXCEEDED`, and "Unexpected error, please try again" only for genuine 500s.

**Principle check:** *Read the error, trust the error.* We didn't build error handling to satisfy a checkbox. We built it so that when something breaks — and something will always break — the error tells you exactly what happened, exactly where, and exactly why. The domain throws specific errors with structured context. The use case doesn't interfere. The controller delegates. The error handler translates. The frontend displays. No layer swallows. No layer guesses. No layer adds a fallback that hides a bug. Errors are communication. Treat them that way.
...
Look at the fields. `currentStatus: "BOUGHT"` -- that's where the item is now. `targetStatus: "APPROVED"` -- that's the status you tried to transition to. The state machine in `item-status.ts` only allows BOUGHT to transition to ARCHIVED. You tried to go backward, and it caught you.
...
This reads the `auth-token` cookie (set during login, which we'll build in Chapter 12) and returns an Authorization header. Every BFF route will call this to forward the user's identity to the backend. If there's no cookie (user not logged in), it returns an empty object -- and that's fine right now. Our backend routes aren't protected yet, so requests without auth headers will still work. Once we add the auth middleware in Chapter 12, unauthenticated requests will get a proper 401, and the frontend can redirect to login.
...
Remember the `getAuthHeaders` function we wrote in `src/lib/backendClient.ts` back in Chapter 10? It reads the `auth-token` cookie and returns an Authorization header. At the time, there was no cookie to read -- the function just returned an empty object. Now the auth chain is complete: the login route sets the cookie, `getAuthHeaders` reads it on every BFF request, and the backend auth middleware verifies it.
...
Create `.dockerignore` in the **project root** (same level as `docker-compose.yml`). Docker reads `.dockerignore` from the build context root, which is `.` in our compose file:
